...
LAVA_WORKER = xxxxxxxxxxx #just the string
DEVICE_ID ?= 0 # to pass as command line argument
...
#	lavacli -i $(LAVA_IDENTITY) devices tags add qemu-zephyr-01 lite
	...
	#-lavacli -i $(LAVA_IDENTITY) device-types add x86_atlas
	#-lavacli -i $(LAVA_IDENTITY) device-types add x86
	#-lavacli -i $(LAVA_IDENTITY) device-types add x86_img24
	
	#-lavacli -i $(LAVA_IDENTITY) devices add --type x86_atlas --worker $(LAVA_WORKER) x86_atlas_01
	#lavacli -i $(LAVA_IDENTITY) devices dict set x86_atlas_01 devices/docker-generic.jinja2
	#lavacli -i $(LAVA_IDENTITY) devices tags add x86_atlas_01 x86_atlas_radar_01

	-lavacli -i $(LAVA_IDENTITY) devices add --type x86 --worker $(LAVA_WORKER) x86_$(DEVICE_ID)
	lavacli -i $(LAVA_IDENTITY) devices dict set x86_$(DEVICE_ID) devices/docker-generic.jinja2
	lavacli -i $(LAVA_IDENTITY) devices tags add x86_$(DEVICE_ID) x86_$(DEVICE_ID)


#       -lavacli -i $(LAVA_IDENTITY) devices add --type mrr --worker $(LAVA_WORKER) mrr-04-x86
#	lavacli -i $(LAVA_IDENTITY) devices dict set mrr-04-192.168.97.50 devices/mrr-04.jinja2
#	lavacli -i $(LAVA_IDENTITY) devices tags add mrr-04-192.168.97.50 mrr-radar-04
#
#       -lavacli -i $(LAVA_IDENTITY) devices add --type mrr --worker $(LAVA_WORKER) mrr-03-x86
#        lavacli -i $(LAVA_IDENTITY) devices dict set mrr-03-x86 devices/docker-generic.jinja2
#        lavacli -i $(LAVA_IDENTITY) devices tags add mrr-03-x86 mrr-x86-radar-03
#
#       -lavacli -i $(LAVA_IDENTITY) devices add --type mrr --worker $(LAVA_WORKER) mrr-02-x86         
#        lavacli -i $(LAVA_IDENTITY) devices dict set mrr-02-x86 devices/docker-generic.jinja2
#        lavacli -i $(LAVA_IDENTITY) devices tags add mrr-02-x86 mrr-x86-radar-02
#
#       -lavacli -i $(LAVA_IDENTITY) devices add --type mrr --worker $(LAVA_WORKER) mrr-01-x86         
#        lavacli -i $(LAVA_IDENTITY) devices dict set mrr-01-x86 devices/docker-generic.jinja2
#        lavacli -i $(LAVA_IDENTITY) devices tags add mrr-01-x86 mrr-x86-radar-01
...
testjob:
	lavacli -i $(LAVA_IDENTITY) jobs submit example/sample_docker.job
...
# change the make file according to your setup, it doesnt affect the docker compose setup. use this file inside the hostmachine 
